...
var currentLevel = parseInt(localStorage.getItem("cssdiner-currentLevel"),10) || 0; // Keeps track of the current level Number (0 is level 1)
...
var progress = JSON.parse(localStorage.getItem("cssdiner-progress")) || blankProgress;
...
function resetProgress(){
  ...
  localStorage.setItem("cssdiner-progress",JSON.stringify(progress));
...
function fireRule(rule) {
  ...
  */
  ...
  if(win){
    ...
    trackProgress(currentLevel, "incorrect", rule);
  }
}
...
function trackProgress(levelNumber, type, enteredSelector){
  if(!progress.guessHistory[levelNumber]) {
  ...
  if(type == "incorrect"){
    ...
    levelStats.enteredSelector = enteredSelector; // tja added
    if(levelStats.correct == false) {
...
function loadLevel(){
  ...
  localStorage.setItem("cssdiner-currentLevel",currentLevel);
